...
$query = "SELECT * FROM view_deviceonline";

//predump(dbselect($query));

$counter = 0; $mailsSent = 0;

$onlines = dbselect($query);
foreach($onlines as $online){
	$counter++;
	
	$description = $online['description'];
	$device_code = $online['device_code'];
	$clientname = $online['clientname'];
	$supervisor = $online['supervisor_email'];
	
	$subject = "Entwarnung! i4Vision Device $description ist wieder online";
	$message = "Sie sind als Supervisor hinterlegt für die Organisation: $clientname<br>Wir teilen Ihnen daher auf diesem Weg mit, dass die<br><br><table><tr><td>i4Vision Device</td><td style='width:10%'></td><td>$description</td></tr><tr><td>mit der Kennung</td><td style='width:10%'></td><td>$device_code</td></tr></table><br><br>wieder online ist und Heartbeats sendet.<br>Das Gerät ist wieder vollkommen einsatzbereit.<br><br><br>i4Vision - Supervisor Service";

	if(sendmail("web@i4vision.de","i4Vision",$supervisor,$subject,$message)){
		$mailsSent++;
		dbexecute("update device set supervisor_warning = 0 where id = ".$online['id']);
	}
}
